...
К сожалению, такой геном порождает не самое сложное поведение, а также остаются некоторые спорные моменты с выполнением нескольких завершающих команд за ход. Есть мысль совместить органеллы и старый геном в виде набора команд, но пока неясно, как это сделать
Но понятно, что текущий геном не поддерживает многоклеточность, и его следует сменить на более подходящий
...
В настоящий момент бот может выполнить только одну завершающую команду за ход. Есть идея дать ботам возможность выполнять несколько действия за один ход, но за каждое следующее потребление энергии будет увеличиваться(вероятно, квадратично)
